...
function buildPrompt(instruction, mode, genre, tone, outputLang, person, person3rdName, lengthMode, tense, userMessage) {
    ...
    if (hasUserMsg) {
        ...
        p += `${userMessage.trim()}\n\n`;
        p += `[핵심 규칙] 위 텍스트는 ${user}가 이미 직접 쓴 부분입니다. 이 내용을 절대 반복하지 마세요. 당신의 출력은 위 텍스트의 마지막 글자 바로 뒤에 붙습니다. 문장 중간이면 그 문장을 이어서 완성하고, 문장이 끝난 상태면 다음 문장부터 시작하세요.\n\n`;
    }
    ...
    if (hasUserMsg) {
        p += `[이어쓰기] 바로 위에 ${user}가 이미 작성한 텍스트가 있습니다. 당신의 출력 첫 글자가 그 텍스트의 마지막 글자 바로 뒤에 자연스럽게 붙어야 합니다. 이미 쓴 부분을 절대 반복하지 마세요.\n`;
    }
